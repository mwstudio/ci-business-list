    ...
    <?php foreach($packages as $package):?>
        ...
        <div class="span4 <?php if($my_package_id == $package->id){ echo "text-success"; }?>" >
        <?php if($my_package_id == $package->id):?>
            <div class="my">&nbsp;</div>
        <?php endif;?>
            <div class="hero-unit text-default">
                <h1><?= $package->type?></h1>
                <?= $package->template?>
                <?php if($my_package_id != $package->id):?>
                <a href="<?= base_url('/packages/activate/'.$package->id)?>" class="btn btn-default">Select</a>
                <?php else:?>
                <a href="javacript:void(0)" class="btn btn-success">Selected</a>
                <?php endif;?>
            </div>
        </div>
    <?php endforeach;?>

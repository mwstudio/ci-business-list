<style>
    ...
    <div class="row">
        ...
        <div class="span12">
            ...
            <div class="hero-unit">
                <p align="j"><?= $business->description?></p>
                <span class="star-rating">
                <?php for($i=1;$i<=5;$i++):?>
                  <input type="radio" name="rating_<?= $i?>" value="<?= $i;?>" id="<?= $business->id?>" <?= ($rating == $i) ? 'checked' : ''?> ><i></i>
                <?php endfor;?>
                </span>
            </div>
        </div>
        ...
        <div class="">
            <h3>Comment</h3>
                ...
                <div class="span6">
                <form class="" method="post" action="<?= base_url('/business/comment')?>">
                <input type="hidden" name="business_id" value="<?= $business->id ?>">
                    <span class="star-rating">
                    <?php for($i=1;$i<=5;$i++):?>
                      <input type="radio" name="rating" value="<?= $i;?>" id="<?= $business->id?>" ><i></i>
                    <?php endfor;?>
                    </span>
                ...
                <div class="span6">
                    <div class="row">
                    <?php foreach($business->review as $review):?>
  ...
  height: 40px;
  width: 40px;
  -webkit-border-radius: 20px;" src="https://lh3.googleusercontent.com/uFp_tsTJboUY7kue5XAsGA=w48-c-h48-rw" alt="The Pulpit Rock" width="30" height="30">
                            </figure>
                            <span style="font-size:20px;  cursor: pointer;font-weight: 500;"><a><?= $review->title?></a></span>
                            <p><?= $review->comment?></p>
                        </div>
                    <?php endforeach; ?>
                    </div>
     ...
     <script>
        $(':radio').change(function(){
            ...
            //post_rating(this.id,this.value);
        });

        
        var post_rating = function(id,value){
            var url ="business/rating/";
            $.post(
                url,
                {id,value}
            ).done(function(data){
                ...
                /*if(!data.success){
                    if(data.error == 'login'){
                        window.location = "<?= base_url('/signin/?redirect='.current_url())?>/rating/"+data.post.id+'/'+data.post.value;
                    }
                    if(data.error == 'arg'){
